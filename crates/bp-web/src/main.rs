// Fallback HTML shown when the financial report hasn't been generated yet.
// On Fly, the real report lives at $DATA_DIR/report.html (generated by validator-accounting).
#[cfg(feature = "ssr")]
static FINANCIALS_FALLBACK: &str = concat!(
    "<!DOCTYPE html><html><body style='font-family:monospace;padding:2em'>",
    "<h1>Report not yet generated</h1>",
    "<p>Run <code>validator-accounting</code> on this machine to generate the financial report.</p>",
    "<p>See DEPLOY_FLY.md for instructions.</p>",
    "</body></html>"
);
...
async fn financials_handler(headers: axum::http::HeaderMap) -> axum::response::Response {
    ...
    // Read report from volume at runtime; fall back to placeholder if not generated yet
    let data_dir = std::env::var("DATA_DIR").unwrap_or_else(|_| "./data".to_string());
    let report_path = format!("{}/report.html", data_dir);
    let html = std::fs::read_to_string(&report_path).unwrap_or_else(|_| FINANCIALS_FALLBACK.to_string());
